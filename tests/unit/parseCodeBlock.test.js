...
describe("parseCodeBlock tests", () => {
  ...
  test("should return a string with <pre> and <code> tags if part of string is encompassed between ```", () => {
    ...
    expect(parseCodeBlock(testSentenceWithTicks)).toBe(
      `<pre>\n\t\t\t\t<code>${testSentence}\n\t\t\t\t</code>\n\t\t\t</pre>`
    );
  });

  test("should return a string without <pre> and <code> tags if part of string is encompassed between `` and ``` or vice versa", () => {
    const testSentence =
      "\nThis is an fenced code block\nThis is a second line in the fenced block\n";
    let testSentenceWithTicks = `\`\`${testSentence}\`\`\``;
    expect(parseCodeBlock(testSentenceWithTicks)).not.toBe(
      `<pre>\n\t\t\t\t<code>${testSentence}\n\t\t\t\t</code>\n\t\t\t</pre>`
    );
    testSentenceWithTicks = `\`\`\`${testSentence}\`\``;
    expect(parseCodeBlock(testSentenceWithTicks)).not.toBe(
      `<pre>\n\t\t\t\t<code>${testSentence}\n\t\t\t\t</code>\n\t\t\t</pre>`
    );
  });
  ...
  test("should return a string without <code> tags if part of string is encompassed between ``` and `` or vice versa", () => {
    expect(parseCodeBlock("```sentence 1``")).not.toBe(
  ...
  test("should throw error if anything except a string is received", () => {
    [null, 4, undefined].forEach((input) => {
      expect(() => parseCodeBlock(input)).toThrow(
        new Error("Invalid input: text must be a string")
      );
    });
  });
});
